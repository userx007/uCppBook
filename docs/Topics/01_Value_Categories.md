...
std::string getName() { return "Bob"; }
std::string s = getName();  // getName() returns prvalue
...
An **xvalue** has identity but is about to expire (can be moved from). Created primarily through `std::move` or functions returning rvalue references.
...
std::vector<int> v = std::move(getVector());  // std::move(...) is xvalue

// Casting to rvalue reference creates xvalue
std::string s = "World";
std::string s2 = static_cast<std::string&&>(s);  // cast result is xvalue
```
...
int&& ref = std::move(x);     // std::move(x) is glvalue (specifically xvalue)
...
```cpp
void process(std::string&& s) {  // accepts rvalues
    std::cout << s << '\n';
}

process("temporary");             // OK: string literal converts to prvalue
process(std::string("temp"));     // OK: prvalue
process(std::move(str));          // OK: xvalue

// rvalue references bind to rvalues
std::string&& rref = std::string("movable");  // OK
// std::string&& rref2 = str;     // Error: str is lvalue
```
...
Buffer b3 = std::move(b1);        // Move: std::move(b1) is xvalue
...
| xvalue | Yes | Yes | `std::move(x)`, `std::forward<T>(x)` |
