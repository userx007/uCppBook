I'll provide you with a comprehensive guide to memory ownership models in C++, which is crucial for writing safe and efficient code.I've created a comprehensive guide covering all major memory ownership models in C++. Here's what's included:

## **Core Ownership Models:**

1. **Unique Ownership (`std::unique_ptr`)** - Single owner, automatic deletion, zero overhead, ownership transferable via `std::move()`

2. **Shared Ownership (`std::shared_ptr`)** - Multiple owners via reference counting, object deleted when last owner destroyed

3. **Weak References (`std::weak_ptr`)** - Non-owning observers that don't affect reference count, essential for breaking circular dependencies

4. **Non-Owning References** - Raw pointers/references for observation without ownership responsibility

## **Practical Patterns Demonstrated:**

- Factory functions that transfer ownership
- Container ownership management
- Circular reference problems and solutions (parent-child relationships)
- Custom deleters for special cleanup needs

## **Key Takeaways:**

- **Prefer `unique_ptr` by default** - it has zero runtime overhead and clear semantics
- **Use `make_unique`/`make_shared`** - exception-safe and efficient
- **Never use raw `new`/`delete`** - always rely on smart pointers (RAII principle)
- **Use `weak_ptr` for back-references** to prevent circular dependencies
- **Raw pointers are fine for non-owning access**, just ensure the owner manages lifetime

The code includes working examples you can compile and run (requires C++14 or later). Each section demonstrates proper usage patterns and common scenarios you'll encounter in real C++ applications.
```cpp
#include <iostream>
#include <memory>
#include <vector>
